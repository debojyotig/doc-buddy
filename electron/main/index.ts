...
function loadConfig() {
  ...
  if (!config) {
    ...
    // Set environment variables even if validation fails (allow partial config)
    // Datadog credentials
    if (config.datadog) {
...
// Console interceptor to send logs to renderer
function interceptConsole(window: BrowserWindow) {
